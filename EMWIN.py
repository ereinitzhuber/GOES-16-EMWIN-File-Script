#Simple script to concatenate all EMWIN text messages in a directory to a single file.
...
class ProductIDs(Enum):
    ...
    GRE	 = "GREEN"
    HAD  = "Tropical Cyclone Discussion, Atlantic, Caribbean, Gulf of Mexico"
    HAP  = "Public Tropical Cyclone, Atlantic, Caribbean, Gulf of Mexico"
    HAF  = "Forecast/Advisory Tropical Cyclone, Atlantic, Caribbean, Gulf of Mexico"
    HAS  = "TROPICAL WEATHER SUMMARY NORTH ATLANTIC"
    HAW  = "TROPICAL / SUBTROPICAL WEATHER OUTLOOK NORTH ATLANTIC"
    HED  = "Tropical Cyclone Discussion, Eastern North Pacific"
    HEF  = "Forecast/Advisory Tropical Cyclone, Eastern North Pacific"
    HEP  = "Public Tropical Cyclone, Eastern North Pacific"
    HET  = "Tropical Weather Discussion"
    HEW  = "TROPICAL / SUBTROPICAL WEATHER OUTLOOK E. NORTH PACIFIC"
    HAT  = "Tropical Weather Discussion NORTH ATLANTIC"
    ...
    HNP  = "Public Tropical Cyclone & Forecast/Advisory Tropical Cyclone, Honolulu, HI"
    HNS  = "TROPICAL WEATHER SUMMARY E. NORTH PACIFIC"
    ...
    NWX  = "HOURLY TEMPERATURE/WEATHER"
...
class Message():
    def __init__(self, prod, path, fname, text):
        self.product = prod
        self.path = path
        self.fileName = fname
        self.text = text
...
for key in files.keys():
    try:
        ...
        for line in fileObject:
            msgstr += line
        fileObject.close()
        prodRaw = re.findall("[A-Z]+", files[key])
        prod = prodRaw[5][:3]
        msg = Message(prod, key, files[key], msgstr)
        messages.append(msg)
    except UnicodeDecodeError:
        continue


handler = open(path + "\\EMWIN.txt", "w")

for typ in ProductIDs:
    tempList = []
    for msg in messages:
        if msg.product == typ.name:
            tempList.append(msg)
    if len(tempList) > 0:
        handler.write('\n\n#########################################################################################################################\n')
        handler.write(f'{typ.name}: {typ.value}\n')
        handler.write('#########################################################################################################################\n\n\n')
    for msg in tempList:
        strMsg = msg.fileName + "\n\n\n"
        strMsg += msg.text
...
tempList = []
for msg in messages:
    if msg.product not in ProductIDs.__members__:
        tempList.append(msg)
if len(tempList) > 0:
    handler.write('\n\n#########################################################################################################################\n')
    handler.write(f'Misc\n')
    handler.write('#########################################################################################################################\n\n\n')
for msg in tempList:
    strMsg = msg.fileName + "\n\n\n"
    strMsg += msg.text
